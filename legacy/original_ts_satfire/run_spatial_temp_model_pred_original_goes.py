...
from original_ts_satfire.data_generator_pred_goes_subdaily_original import FireDatasetWithGOESSubdaily
...
def goes_train_path(root_path, split, ts_length, interval):
    candidates = [
        os.path.join(root_path, f"dataset_{split}", f"pred_{split}_goes_subdaily_seqtoseq_alll_{ts_length}i_{interval}.npy"),
        os.path.join(root_path, f"GOES_{split}", f"pred_{split}_goes_subdaily_seqtoseq_alll_{ts_length}i_{interval}.npy"),
    ]
    for path in candidates:
        if os.path.exists(path):
            return path
    raise FileNotFoundError(f"No GOES sub-daily {split} file found. Tried: {candidates}")
...
def build_model(model_name, n_channel, num_classes, image_size, window_size, num_heads, hidden_size):
    if model_name != "swinunetr3d_goes_subdaily":
        raise NotImplementedError("Use -m swinunetr3d_goes_subdaily for this original-style GOES runner.")
    patch_size = (1, 2, 2)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Original-style TS-SatFire pred runner with GOES bottleneck fusion.')
    ...
    checkpoint_dir = os.environ.get("TS_SATFIRE_ORIG_GOES_CHECKPOINT_ROOT", "/home/jlc3q/data/SatFire/checkpoints/original_ts_satfire_goes")
    ...
    if not train:
        ...
        train_dataset = FireDatasetWithGOESSubdaily(image_path=image_path, label_path=label_path, goes_subdaily_path=goes_path, ts_length=ts_length, n_channel=n_channel, target_is_single_day=target_is_single_day, use_augmentations=True)
        train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
        val_dataset = FireDatasetWithGOESSubdaily(image_path=val_image_path, label_path=val_label_path, goes_subdaily_path=val_goes_path, ts_length=ts_length, n_channel=n_channel, target_is_single_day=target_is_single_day, use_augmentations=False)
    ...
    if train or test_after_train:
        ...
        for i, id in enumerate(ids):
            ...
            try:
                ...
                skipped_missing_ids += 1
                print(f"Skipping missing GOES subdaily test array: {id}. {exc}")
                continue

            test_dataset = FireDatasetWithGOESSubdaily(image_path=test_image_path, label_path=test_label_path, goes_subdaily_path=test_goes_path, ts_length=ts_length, n_channel=n_channel, label_sel=0, target_is_single_day=True, use_augmentations=False)
